...
def get_user_by_email(db: Session, email: str):
    try:
        query = text("""SELECT usuario.id_usuario,usuario.nombre_completo, usuario.identificacion, usuario.id_rol,rol.nombre AS nombre_rol,
...
def modify_status_user(db:Session,user_id:int):
    try:
        query = text("""
                        UPDATE usuario SET estado = IF(estado, FALSE, TRUE) WHERE id_usuario = :id
                     """)
...
def get_users_by_centro(db: Session, cod_centro: int):
    try:
        query = text("""
            SELECT usuario.id_usuario, usuario.nombre_completo, usuario.identificacion, usuario.id_rol,rol.nombre AS nombre_rol
                   usuario.correo, usuario.tipo_contrato, usuario.telefono, usuario.estado, usuario.cod_centro
            FROM usuario
            JOIN rol ON usuario.id_rol = rol.id_rol
